...
@section('content-header')
    <h1>
        {{ trans('appuser::appusers.title.create appuser') }}
    </h1>
    <ol class="breadcrumb">
        <li><a href="{{ route('dashboard.index') }}"><i class="fa fa-dashboard"></i> {{ trans('core::core.breadcrumb.home') }}</a></li>
        <li><a href="{{ route('admin.appuser.appuser.index') }}">{{ trans('appuser::appusers.title.appusers') }}</a></li>
        <li class="active">{{ trans('appuser::appusers.title.create appuser') }}</li>
    </ol>
@stop

@section('content')
    {!! Form::open(['route' => ['admin.appuser.appuser.store'], 'method' => 'post']) !!}
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="tab-content">
                    ...
                    @foreach (LaravelLocalization::getSupportedLocales() as $locale => $language)
                        ...
                        <div class="tab-pane {{ locale() == $locale ? 'active' : '' }}" id="tab_{{ $i }}">
                            @include('appuser::admin.appusers.partials.create-fields', ['lang' => $locale])
                        </div>
                    @endforeach
